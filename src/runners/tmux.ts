	...
	async function runSession(
		...
		while (true) {
			...
			if (timeoutMs !== undefined) {
				try {
					const activityMtimeMs = (await stat(eventPath)).mtimeMs;
					if (activityMtimeMs > lastActivityMtimeMs) {
						lastActivityMtimeMs = activityMtimeMs;
						lastActivityAt = Date.now();
					}
				} catch {
					// The worker may not have created the event file yet.
				}
				if (Date.now() - lastActivityAt >= timeoutMs) stopKind = "timeout";
			}
			if (stopKind !== null) {
				await killTmuxSession(tmuxIdentity);
				return {
