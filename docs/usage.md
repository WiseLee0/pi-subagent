...
The extension runs opportunistic maintenance at startup, before a new run, and before opening the panel. Maintenance is globally locked and throttled to once per 24 hours. Like explicit prune, it checks physical path containment, validates every run/attempt/task status, and deletes under the per-run lock with an exact `updatedAt` generation check and atomic rename; it does not bypass the prune safety fence. Only terminal runs with no active attempt are eligible:
...
| `timeoutMs` | Stop a `run` after this much worker inactivity; defaults to 300,000 ms (5 minutes) and resets whenever the worker emits stdout/stderr or an inline SDK event. For `action: "wait"`, it limits polling duration and defaults to 60 seconds. |
...
- `timeoutMs` on a run is the worker inactivity timeout. If omitted, pi-subagent defaults it to 300,000 ms (5 minutes). The timer resets whenever a process-backed worker emits stdout/stderr or an inline worker emits an SDK event; an explicit value overrides the default.
...
| `PI_CODING_AGENT_DIR` | `~/.pi/agent` | Pi's source agent directory; sandboxed runners copy configuration into a private writable overlay and grant lock paths there, not in the parent agent directory. |
